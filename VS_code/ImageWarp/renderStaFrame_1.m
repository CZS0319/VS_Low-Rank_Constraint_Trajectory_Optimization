function [ori_array,imwarp_array,new_array]=renderStaFrame_1(img_array,new_x_motion_meshes,new_y_motion_meshes,gap,path,path_syn,scale)
% renderStaFrames():逐帧渲染图像序列,生成稳定图像,改进两个评价指标D和C_R
% input
%  
% output
% 
...
parfor i=1:im_len
    ...
    % 保存扭曲后还原原始图像大小的图像
    new_array{i}=imwarp_new;
    % 保存原始图像的局部区域
%     height_I=round(size(I,1)/2);
%     width_I=round(size(I,2)/2);
%     H=round(im_height*scale/2);
%     W=round(im_width*scale/2);
%     ori_array{i}=I(height_I-H+1:height_I+H,width_I-W+1:width_I+W,:);
%     imwarp_ori=imwarp(height_center-height_I+1:height_center+height_I,width_center-width_I+1:width_center+width_I,:);
    ...
    syn_im=cat(2,I,syn,imwarp_new);
    imwrite(syn_im,syn_path);
end
delete(p);
% toc;
